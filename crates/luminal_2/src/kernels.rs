...
pub fn make_tiled_matmul_basic(
    ...
    acc = loop_in(acc, Expression::from(k) / 8, Term::Acc("Acca"), &mut graph);
    ...
    out = loop_out(out, Expression::from(k) / 8, Term::Acc("Acca"), &mut graph);
...
mod tests {
    ...
    fn test_matmul() {
        ...
        .into_flattened();
        let outputs = run_graph(vec![a, b], &kernels).pop().unwrap();
        let pt_output = vec![
    ...
    fn test_tiled_matmul_basic() {
        ...
        let b = vec![
            [
                -0.8834, -0.4189, -0.8048, 0.5656, 0.6104, 0.4669, 1.9507, -1.0631, -0.0773,
                0.1164, -0.5940, -1.2439, -0.1021, -1.0335, -0.3126, 0.2458, -0.2596, 0.1183,
                0.2440, 1.1646, 0.2886, 0.3866, -0.2011, -0.1179, 0.1922, -0.7722, -1.9003, 0.1307,
                -0.7043, 0.3147, 0.1574, 0.3854,
            ],
            [
                0.9671, -0.9911, 0.3016, -0.1073, 0.9985, -0.4987, 0.7611, 0.6183, 0.3140, 0.2133,
                -0.1201, 0.3605, -0.3140, -1.0787, 0.2408, -1.3962, -0.0661, -0.3584, -1.5616,
                -0.3546, 1.0811, 0.1315, 1.5735, 0.7814, -1.0787, -0.7209, 1.4708, 0.2756, 0.6668,
                -0.9944, -1.1894, -1.1959,
            ],
            [
                ...
                -1.2537, 0.9868, -0.4947, -1.2830, 0.9552, 1.2836, -0.6659, 0.5651, 0.2877,
                -0.0334, -1.0619, -0.1144,
            ],
            [
                -0.3433, 1.5713, 0.1916, 0.3799, -0.1448, 0.6376, -0.2813, -1.3299, -0.1420,
                -0.5341, -0.5234, 0.8615, -0.8870, 0.8388, 1.1529, -1.7611, -1.4777, -1.7557,
                0.0762, -1.0786, 1.4403, -0.1106, 0.5769, -0.1692, -0.0640, 1.0384, 0.9068,
                -0.4755, -0.8707, 0.1447, 1.9029, 0.3904,
            ],
            [
                -0.0394, -0.8015, -0.4955, -0.3615, 0.5851, -1.1560, -0.1434, -0.1947, -0.0856,
                1.3945, 0.5969, -0.4828, -0.3661, -1.3271, 1.6953, 2.0655, -0.2340, 0.7073, 0.5800,
                0.2683, -2.0589, 0.5340, -0.5354, -0.8637, -0.0235, 1.1717, 0.3987, -0.1987,
                -1.1559, -0.3167, 0.9403, -1.1470,
            ],
            [
                0.5588, 0.7918, -0.1847, -0.7318, -0.0807, -0.9801, 0.0605, -0.4890, -0.8137,
                0.8200, -0.6332, 1.2948, 1.4628, -0.6204, 0.9884, -0.4322, -0.6232, -0.2162,
                -0.4887, 0.7870, 0.1076, -1.0715, -0.1166, -1.0170, -1.1980, 0.4784, -1.2295,
                -1.3700, 1.5435, -0.0332, -0.4186, -0.2556,
            ],
            [
                ...
                0.3827, -0.5497, -0.9940, 1.3459, 1.9457, -1.2904, -2.3495, -2.0689, 0.9094,
                -0.6946, 1.9595, -1.1038,
            ],
            [
                0.5411, 1.5390, 1.0860, 1.2464, 0.1151, 1.6193, 0.4637, 1.3007, 0.8732, 0.0651,
                0.7732, -0.9701, -0.8877, -0.3183, -0.3344, 0.4543, 0.4990, 0.8780, 0.3894, 1.4625,
                0.4795, -0.5334, -0.0347, 0.6573, -0.3112, -0.5620, -0.4835, -1.2721, -0.1740,
                0.5541, -0.1817, -0.2345,
            ],
        ]
        .into_flattened();
        let outputs = run_graph(vec![a, b], &kernels).pop().unwrap();
