...
<img
  ...
  alt="Image credit: https://www.exxactcorp.com/"
/>
...
1) Swap out each primop with a backend-specific primop.
2) Add in operations to copy to device and copy from device before and after Function ops.
3) Pattern-match to swap out chunks of ops with specialized variants.
4) All other optimizations.

So let's go through how we do this for the Metal backend to support Apple GPUs.
...
```
and their entire model now runs on the GPU!
