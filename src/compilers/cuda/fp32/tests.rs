...
fn test_log2() {
    ...
    cx.compile(CudaFp32Compiler::default());
    cx.execute();
...
fn test_log2exp2() {
    ...
    cx.compile(<(GenericCompiler, CudaFp32Compiler)>::default());
    cx.execute();

    assert_close(&b.data(), &[1., 2., 3.]);
}
...
fn test_square() {
    ...
    cx.compile(<(CudaFp32Compiler, GenericCompiler)>::default());
    cx.execute();
...
fn test_max_reduce() {
    ...
    let a = cx.new_tensor::<R3<2, 2, 3>>("Input").set(data.clone());
    let b = a.max_reduce::<_, LAxis<1>>().retrieve();
    let c = a.max_reduce::<_, LAxis<0>>().retrieve();
    let d = a.max_reduce::<_, LAxis<2>>().retrieve();
...
fn test_relu_and_linear() {
    ...
    assert_close(&uncompiled_b, &out.as_vec());
}
