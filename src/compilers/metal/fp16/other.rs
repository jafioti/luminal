...
#include <metal_stdlib>
using namespace metal;
kernel void metal_gather(device float *inp [[buffer(0)]], device half *weights [[buffer(1)]], device half *out [[buffer(2)]], device int& n_embeddings [[buffer(3)]], device int& embedding_dim [[buffer(4)]], uint2 i_ [[thread_position_in_grid]]) {
...
impl Compiler for MetalGatherCompiler {
    fn compile(&self, graph: &mut Graph) {
        ...
        let s: SelectEdge = SelectOp::new()
            .check(|op, _| {
                if let Some(op) = op.as_any().downcast_ref::<crate::op::Function>() {
                    op.0 == "Gather"
                } else {
                    false
                }
        ...
        for _ in s.search(graph) {
            ...
            let new_gather = graph
                .add_op(MetalGather::new(
                    dev.clone(),
                    weights_copy_from.2.shape()[1].to_usize().unwrap(),
                ))
